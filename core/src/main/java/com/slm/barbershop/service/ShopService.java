...
public class ShopService extends ServiceImpl<ShopMapper, Shop> {
    ...
    public Shop update(Long id, ShopRequest request, Long userId) {
        ...
        if (!shop.getUserId().equals(userId)) {
            throw new BizException(ResultStatus.PERMISSION_DENIED, "无权限修改此店铺");
        }
    ...
    public void delete(Long id, Long userId) {
        ...
        if (!shop.getUserId().equals(userId)) {
            throw new BizException(ResultStatus.PERMISSION_DENIED, "无权限删除此店铺");
        }
    ...
    private void validateBusinessHours(ShopRequest request) {
        ...
        if (request.getOpenTime() != null && request.getCloseTime() != null
            ...
            throw new BizException(ResultStatus.BAD_REQUEST, "营业结束时间必须晚于或等于营业开始时间");
        }
        if (request.getWeeklyOff() != null && request.getWeeklyOff().size() > 7) {
            throw new BizException(ResultStatus.BAD_REQUEST, "周内休息标记最多 7 个元素");
        }
    }
