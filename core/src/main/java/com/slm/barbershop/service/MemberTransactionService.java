...
public class MemberTransactionService extends ServiceImpl<MemberTransactionMapper, MemberTransaction> {
    ...
    public MemberTransaction doUpdate(Long memberId,
        ...
        if (type == TransactionType.CONSUME) {
            if (employeeId == null) {
                throw new BizException(ResultStatus.BAD_REQUEST, "消费必须选择员工");
            }
        ...
        if (type == TransactionType.CONSUME) {
            if (effectiveItems.isEmpty()) {
                throw new BizException(ResultStatus.BAD_REQUEST, "消费必须包含至少一个服务项目");
            }
            ...
            for (int i = 0; i < effectiveItems.size(); i++) {
                ...
                if (req.getItemId() == null) {
                    throw new BizException(ResultStatus.BAD_REQUEST, "消费项目ID不能为空");
                }
                Integer qty = req.getQuantity() == null ? 1 : req.getQuantity();
                if (qty < 1 || qty > 999) {
                    throw new BizException(ResultStatus.BAD_REQUEST, "消费项目数量必须在 1~999 之间");
                }
                ServiceItem item = itemMap.get(req.getItemId());
                if (item == null) {
                    throw new BizException(ResultStatus.BAD_REQUEST, "消费项目不存在或不属于当前店铺");
                }
                if (item.getStatus() == null || item.getStatus() != 1) {
                    throw new BizException(ResultStatus.BAD_REQUEST, "消费项目已下架:" + item.getName());
                }
                BigDecimal unitPrice = req.getUnitPrice() != null ? req.getUnitPrice() : item.getPrice();
                if (unitPrice == null || unitPrice.compareTo(BigDecimal.ZERO) <= 0) {
                    throw new BizException(ResultStatus.BAD_REQUEST, "消费项目价格无效:" + item.getName());
                }
        ...
        if (finalAmount == null || finalAmount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new BizException(ResultStatus.BAD_REQUEST, "交易金额必须大于0");
        }
