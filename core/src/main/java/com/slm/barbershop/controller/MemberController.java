...
public class MemberController {
    ...
    public ApiResponse<MemberResponse> create(@RequestBody @Valid MemberRequest request) {
        ...
        return ApiResponse.success(memberConverter.toResponse(member));
    }
    ...
    public ApiResponse<MemberResponse> getById(@PathVariable Long id, @RequestParam Long shopId) {
        Member member = memberService.getByIdAndShopId(shopId, id).orElseThrow(() -> new BizException(ResultStatus.SOURCE_NOT_FOUND, "会员不存在"));
        return ApiResponse.success(memberConverter.toResponse(member));
    }
    ...
    public ApiResponse<PageResult<MemberResponse>> page(@Validated MemberQuery query) {
        return ApiResponse.success(PageResult.map(memberService.page(query), memberConverter::toResponse));
    }
    ...
    public ApiResponse<MemberTransactionResponse> store(@PathVariable Long id, @RequestBody MemberTransactionRequest request) {
        ...
        return ApiResponse.success(transactionConverter.toResponse(transaction));
    }
    ...
    public ApiResponse<List<MemberTransactionResponse>> transactions(@PathVariable Long id) {
        assertMemberSelf(id);
        return ApiResponse.success(transactionService.listByMemberId(id));
    }
    ...
    public ApiResponse<List<MemberMatchVO>> matchForLogin(
        ...
        if (phone == null || phone.trim().isEmpty()) {
            throw new BizException(ResultStatus.BAD_REQUEST, "手机号不能为空");
        }
        return ApiResponse.success(memberService.match(phone));
    }
    ...
    public ApiResponse<PageResult<MemberTransactionResponse>> transactionsPage(
        ...
        assertMemberSelf(id);
        return ApiResponse.success(transactionService.pageByMemberId(id, query));
    }
    ...
    private void assertMemberSelf(Long pathMemberId) {
        ...
        if (user.isMember() && !user.getId().equals(pathMemberId)) {
            throw new BizException(ResultStatus.PERMISSION_DENIED, "无权访问该会员数据");
        }
    }
