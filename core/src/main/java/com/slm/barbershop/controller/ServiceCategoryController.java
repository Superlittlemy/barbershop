...
public class ServiceCategoryController {
    ...
    public ApiResponse<ServiceCategoryResponse> create(@RequestBody @Valid ServiceCategoryRequest request) {
        ...
        return ApiResponse.success(categoryConverter.toResponse(category));
    }
    ...
    public ApiResponse<ServiceCategoryResponse> getById(@PathVariable Long id) {
        ServiceCategory category = categoryService.getOptById(id).orElseThrow(() -> new BizException(ResultStatus.SOURCE_NOT_FOUND, "分类不存在"));
        return ApiResponse.success(categoryConverter.toResponse(category));
    }
    ...
    public ApiResponse<PageResult<ServiceCategoryResponse>> page(@Validated ServiceCategoryQuery query) {
        return ApiResponse.success(PageResult.map(categoryService.page(query), categoryConverter::toResponse));
    }
